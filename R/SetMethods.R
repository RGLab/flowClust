...
function(object)
{
    ...
    cat("expName, varNames, K, mu, sigma, w, z, u, label, uncertainty, ruleOutliers, flagOutliers, rm.max, rm.min, lambda, nu, logLike, BIC, ICL\n")
}
)
...
#    if (object@ruleOutliers[1]==0) cat(round(object@ruleOutliers[2]*100, 2), "% quantile")  else cat("Cutoff at ", round(object@ruleOutliers[2], 2))
#    if (object@ruleOutliers[3]=="0") cat("\n")  else cat(", probability of assignment >= ", round(object@ruleOutliers[3], 2), "\n")
...
setMethod("show", signature("tmixFilter"),
    function(object) {
        cat("A t-mixture filter named '", object@filterId, "'")
        ...
        cat("\n")
        cat("Rule of identifying outliers: ")
        if (is.na(object@u.cutoff)) cat(round(object@level*100, 2), "% quantile")  else cat("Cutoff at ", round(object@u.cutoff, 2))
...
#        if (missing(select)) object@parameters[[1]] <- x@parameters[[1]]  else object@parameters[[1]] <- select
        object
    }
)
...
setReplaceMethod("ruleOutliers", signature("flowClust","list"),
    function(object,value=list(level=NULL, u.cutoff=NULL, z.cutoff=NULL)) {
        ...
        if (object@ruleOutliers[1]==0) {     # 0 means quantile
            p <- ncol(object@mu)
            cc <- p * qf(object@ruleOutliers[2], p, object@nu)
            u.cutoff <- (object@nu+p)/(object@nu + cc)
