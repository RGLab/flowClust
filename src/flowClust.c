...
void flowClust(double *y, int *ly, int *py, double *mu, double *precision, double *w, double *z, double *u, double *lambda, int *label, double *uncertainty, double *cutoff, double *z_cutoff, int *flagOutliers, int *K, double *nu, int *B, double *tol, int *transform, double *logLike)
{
	...
	gsl_vector *SumZ=gsl_vector_calloc(*K), *SumWZ=gsl_vector_calloc(*K), *SumWY=gsl_vector_calloc(*py);
...
/*			if(status!=0)
			{
				warning("Brent's algorithm returned an error, error code %d\n", status);				
				 ...
				 // fprintf(stderr, "Warning, Brent's algorithm returned an error, error code %d\n", status);
*/                
		}
...
/*			if((status!=0) && (iterSolve<iterSolveMax))
        ...
        // fprintf(stderr, "Warning in the inner loop, Brent's algorithm returned an error, error code %d\n", status);                    
			}
*/			/* Set the new value of the transformation */
	...
	for(i=0;i<*ly;i++)
	{
		...
		if(gsl_matrix_get(&Weight.matrix,i,label[i]-1)<*cutoff || gsl_matrix_get(&Z.matrix,i,label[i]-1)<*z_cutoff)
			flagOutliers[i]=1;
		else
			flagOutliers[i]=0;
	}
	...
	{        
		...
		*logLike+=log(like);
		for(k=0;k<K;k++)
		{
			...
			gsl_matrix_set(Z,i,k,gsl_matrix_get(Z,i,k)/normConstant);
			/* Here I assume the cholesky decomposition has been done */
			/* Note that I don't store the weights, I only store WZ */		  
			/* I store the square root for efficiency */
			if(last==0)
			{
				gsl_matrix_set(Weight,i,k,sqrt(gsl_matrix_get(Z,i,k)*(py+nu)/(gsl_pow_2(gsl_mahalanobis(&Prow.matrix, &row1.vector, &row2.vector, 1))+nu)));
				gsl_vector_set(SumZ,k,gsl_vector_get(SumZ,k)+gsl_matrix_get(Z,i,k));
				gsl_vector_set(SumWZ,k,gsl_vector_get(SumWZ,k)+gsl_pow_2(gsl_matrix_get(Weight,i,k)));
				gsl_matrix_set(Z,i,k,gsl_pow_2(gsl_matrix_get(Weight,i,k)));
			}
			else
			{
